...
class BookingService
{
     ...
     * Creates a Conversation + System Message so customers can chat about their booking.
     * NO support case — appointments stay standalone, but messaging is enabled.
     */
    public function createBooking(array $data, $user, ?string $guestSessionId): array
    {
        return DB::transaction(function () use ($data, $user, $guestSessionId) {
            ...
            $booking = ServiceBooking::create([
                'case_id' => null, // <-- NO case linked for standalone bookings
                ...
                'metadata' => [
                    ...
                    'conversation_id' => $conversation->id, // <-- Store conversation ID
                    'ip' => request()->ip(),
                    'user_agent' => request()->userAgent(),
                ],
            ]);

            // ─── Step 3: Create clean system message (single line, no description) ───
            $systemBody = "Service Booking Created " .
                "Service: " . str_replace('_', ' ', $booking->service_type) . " " .
                "Date: " . ($booking->requested_date?->format('M j, Y') ?? 'TBD') . " " .
                "Time: " . ($booking->preferred_time ?? 'TBD') . " " .
                "Location: " . ($booking->shop_location ?? 'TBD') . " " .
            ...
            // ─── Step 4: Notify customer (email/SMS) ───
