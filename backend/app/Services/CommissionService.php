...
class CommissionService
{
    ...
    /**
     * Get user-facing pricing (what user pays only)
     * Users see: bike + add-ons + insurance + transport (no commission breakdown)
     */
    public static function getUserPricing(float $amount, int $sellerId, array $addOns = [], bool $insurance = false, float $deliveryFee = 0): array
    {
        $base = $amount;
        $addOnsTotal = array_sum($addOns);
        $insuranceFee = $insurance ? 200 : 0;

        $subtotal = $base + $addOnsTotal + $insuranceFee + $deliveryFee;

        return [
            'bike_rental' => number_format($base, 2),
            'add_ons' => $addOnsTotal > 0 ? number_format($addOnsTotal, 2) : null,
            'insurance' => $insuranceFee > 0 ? number_format($insuranceFee, 2) : null,
            'delivery' => $deliveryFee > 0 ? number_format($deliveryFee, 2) : null,
    ...
    /**
     * Get admin/lister full payout breakdown
     * Listers/Admins see: income, platform commission, net payout
     */
    public static function getAdminBreakdown(float $amount, int $sellerId): array
    {
        $calc = self::calculate($amount, $sellerId);

        return [
            'total_amount' => number_format($amount, 2),
            'platform_commission_rate' => $calc['commission_rate'] . '%',
            'platform_commission_amount' => 'KES ' . number_format($calc['commission_amount'], 2),
            'lister_income' => 'KES ' . number_format($calc['seller_payout'], 2),
            'currency' => 'KES',
            'raw' => $calc,
        ];
    }
