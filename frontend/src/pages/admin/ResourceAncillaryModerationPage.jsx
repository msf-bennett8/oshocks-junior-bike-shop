...
import {
  Shield, CheckCircle, XCircle, Pause, Play, Package, Search,
  AlertTriangle, Clock, Plus, Eye, Settings, TrendingUp, DollarSign,
  Loader, Ban
} from 'lucide-react';
...
const TABS = [
  { key: 'all', label: 'All Ancillary', icon: Package },
...
const ResourceAncillaryModerationPage = () => {
  const { user } = useAuth();
  ...
  const fetchResources = async () => {
    try {
      ...
      // Filter to only ancillary
      const allResources = response.data?.data?.data || response.data?.data || [];
      const ancillaryResources = allResources.filter(r => r.resource_type === 'ancillary');
      setResources(ancillaryResources);
    } catch (err) {
      console.error('Failed to fetch resources:', err);
      toast.error('Failed to load ancillary items');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleApprove = async (resourceCode) => {
    try {
      ...
      toast.success('Ancillary item approved');
      fetchResources();
      fetchStats();
    } catch (err) {
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      toast.success('Ancillary item rejected');
      fetchResources();
      fetchStats();
    } catch (err) {
  ...
  const handlePause = async (resourceCode) => {
    try {
      ...
      toast.success('Ancillary item paused');
      fetchResources();
      fetchStats();
    } catch (err) {
  ...
  const handleResume = async (resourceCode) => {
    try {
      ...
      toast.success('Ancillary item resumed');
      fetchResources();
      fetchStats();
    } catch (err) {
  ...
  const getStatusBadge = (resource) => {
    if (resource.deleted_at) return { label: 'Deleted', color: 'gray', icon: Ban };
    ...
    return { label: resource.status, color: 'gray', icon: Package };
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Helmet><title>Ancillary Moderation | Admin</title></Helmet>
        ...
        <div className="container mx-auto px-4 py-6">
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Ancillary Moderation</h1>
                <p className="text-gray-600">Manage services & add-ons: nutrition packs, insurance, transport to event areas, and more.</p>
              </div>
            </div>
            <button
              onClick={() => setShowUploadModal(true)}
              className="flex items-center gap-2 px-4 py-2.5 bg-teal-600 text-white rounded-lg font-medium hover:bg-teal-700 transition-all"
            >
              <Plus className="w-4 h-4" />
              Upload New Ancillary
            </button>
          </div>
        </div>
      </div>
      ...
      {stats && (
        <div className="container mx-auto px-4 py-6">
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4">
            {[
              { label: 'Total Ancillary', value: stats.total_ancillary || 0, icon: Package, color: 'blue' },
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          ...
          <div className="p-4 border-b border-gray-200">
            <div className="relative max-w-md">
              ...
              <input
                type="text"
                placeholder="Search ancillary by name, code, or category..."
                value={searchQuery}
          ...
          <div className="px-4 py-2 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
            <p className="text-xs text-gray-500">
              Showing <span className="font-semibold">{resources.length}</span> ancillary items
            </p>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
              <thead className="bg-gray-50 text-gray-600 font-semibold">
                <tr>
                  <th className="px-4 py-3">Ancillary Item</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {loading ? (
                  ...
                  <tr><td colSpan="6" className="px-4 py-8 text-center text-gray-500">No ancillary items found</td></tr>
                ) : (
                  resources.map((resource) => {
                    ...
                    return (
                      <tr key={resource.resource_code} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3">
                          <span className="px-2 py-1 bg-teal-100 text-teal-700 rounded-full text-xs font-medium capitalize">
      ...
      {showRejectModal && selectedResource && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
            <h3 className="text-lg font-bold text-gray-900 mb-2">Reject Ancillary Item</h3>
            ...
            <div className="space-y-4 mb-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., New stock arrived, Service capacity increased"
                  className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 text-sm"
                />
              </div>
            </div>
      ...
      {showUploadModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl shadow-xl max-w-2xl w-full p-6">
            <h3 className="text-lg font-bold text-gray-900 mb-4">Upload New Ancillary Item</h3>
            <p className="text-gray-600 mb-4">
              Use the Resource Upload page for full upload functionality.
            </p>
            <div className="flex gap-3">
              <button
...
export default ResourceAncillaryModerationPage;