...
/**
 * Event Code Generator using Bennett Fibonacci 36th Codec
...
class EventCodeService
{
    ...
    public function generate(?Carbon $timestamp = null): string
    {
        ...
        $typeDigit    = '5'; // Always '5' for cycling events
        ...
        $eventCode = $this->codec->encode($rawInput);

        // Collision check with retry (adds 1 second and regenerates)
        if (DB::table('cycling_events')->where('event_code', $eventCode)->exists()) {
    ...
    /**
     * Parse an event code to extract metadata
     */
    public function parse(string $eventCode): array
    {
        if (strlen($eventCode) !== 12) {
            throw new \InvalidArgumentException("Invalid event code length: expected 12, got " . strlen($eventCode));
        }

        // Decode via Bennett Fibonacci 36th
        $decoded = $this->codec->decode($eventCode);
        ...
        // Date parsing (2 chars: AA=27, AB=28, etc.)
     ...
     */
    public function isValid(string $eventCode): bool
    {
        if (strlen($eventCode) !== 12) return false;
        if (!ctype_alnum($eventCode)) return false;
        // Position 7 decoded should be '5' — we can't check encoded directly
        return true;
    }
