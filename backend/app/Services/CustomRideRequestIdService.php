...
/**
 * Custom Ride Request Code Generator using Bennett Fibonacci 36th Codec
 ...
 * 7     = Type digit (8=custom_ride_request)
 * 8-11  = Random 4-char suffix (A-Z, 0-9)
 */
class CustomRideRequestIdService
{
    ...
    public function generate(?Carbon $timestamp = null): string
    {
        ...
        $typeDigit    = '8'; // Always '8' for custom ride requests
        ...
        $requestCode = $this->codec->encode($rawInput);

        // Collision check with retry
        if (DB::table('custom_ride_requests')->where('request_id', $requestCode)->exists()) {
    ...
    public function parse(string $requestCode): array
    {
        if (strlen($requestCode) !== 12) {
            throw new \InvalidArgumentException("Invalid request code length: expected 12, got " . strlen($requestCode));
        }

        $decoded = $this->codec->decode($requestCode);
    ...
    public function isValid(string $requestCode): bool
    {
        if (strlen($requestCode) !== 12) return false;
        if (!ctype_alnum($requestCode)) return false;
        return true;
    }

    // --- Encoding helpers (same as CommunityPostCodeService) ---
