...
/**
 * Community Post Code Generator using Bennett Fibonacci 36th Codec
 ...
 * 7     = Type digit (7=community_post)
 * 8-11  = Random 4-char suffix (A-Z, 0-9)
 */
class CommunityPostCodeService
{
    ...
    public function generate(?Carbon $timestamp = null): string
    {
        ...
        $typeDigit    = '7'; // Always '7' for community posts
        ...
        $postCode = $this->codec->encode($rawInput);

        // Collision check with retry
        if (DB::table('community_posts')->where('post_code', $postCode)->exists()) {
    ...
    public function parse(string $postCode): array
    {
        if (strlen($postCode) !== 12) {
            throw new \InvalidArgumentException("Invalid post code length: expected 12, got " . strlen($postCode));
        }

        $decoded = $this->codec->decode($postCode);
    ...
    {
        if (strlen($postCode) !== 12) return false;
        if (!ctype_alnum($postCode)) return false;
        return true;
    }

    // --- Encoding helpers (same as EventCodeService) ---
