...
class EventImageUploadService
{
    ...
    // Industry best practice: 10MB max for event images
    ...
    public function validateImage(UploadedFile $file): array
    {
        ...
        // TODO: Integrate ClamAV before production
     ...
     * Stores in: oshocks/events/new_events/{eventCode}/
     */
    public function uploadEventImage(UploadedFile $file, string $eventCode): array
    {
        ...
        try {
            $folder = "oshocks/events/new_events/{$eventCode}";

            $result = $this->cloudinary->uploadFile($file, $folder, [
                'resource_type' => 'image',
                'public_id' => $this->generatePublicId($file, $eventCode),
            ]);
    ...
    /**
     * Upload base64 image for event
     */
    public function uploadBase64EventImage(string $base64String, string $eventCode): array
    {
        try {
            $folder = "oshocks/events/new_events/{$eventCode}";
            ...
            Log::error('Event base64 image upload failed', [
                'event_code' => $eventCode,
                'error' => $e->getMessage(),
            ]);
            return ['success' => false, 'error' => 'Upload failed: ' . $e->getMessage()];
     ...
     */
    public function deleteEventImage(string $publicId): bool
    {
        try {
            return $this->cloudinary->deleteFile($publicId, 'image');
        } catch (Exception $e) {
            Log::error('Event image delete failed', [
                'public_id' => $publicId,
                'error' => $e->getMessage(),
            ]);
            return false;
     ...
     */
    protected function generatePublicId(UploadedFile $file, string $eventCode): string
    {
        ...
        return "{$eventCode}_{$timestamp}_{$random}_{$safeName}";
    }
}
